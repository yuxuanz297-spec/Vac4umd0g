...
while True:
    ...
    if micros() - last_ultrasonic_time > 200000:  # 每200ms测一次（而不是每循环都测）
        ...
        if distance_middle<=20:
            ...
            pwp1=pwp1x*(1/2*exp(-0.002*distance_left**2)-1/2*exp(-0.002*distance_right**2)-exp(-0.02*distance_middle**2)+1)
            pwp2=pwp2x*(1/3*exp(-0.002*distance_left**2)-1/2*exp(-0.002*distance_right**2)-exp(-0.02*distance_middle**2)+1)
            pwp3=pwp4x*(1/2*exp(-0.002*distance_right**2)-1/2*exp(-0.002*distance_left**2)-exp(-0.02*distance_middle**2)+1)
            pwp4=pwp4x*(1/2*exp(-0.002*distance_right**2)-1/2*exp(-0.002*distance_left**2)-exp(-0.02*distance_middle**2)+1)
        print(pwp1)
        print(pwp2)
        print(pwp3)
        print(pwp4)
